...
const Post = ({post}) => {
  ...
  const deletePostHandler = async ()=>{
    try {
      const res = await axios.delete(`https://instagramclone-mern-project.onrender.com/api/v1/post/delete/${post._id}`,{withCredentials:true})
      if(res.data.success){
  ...
  const likeOrDislikeHandler = async ()=>{
    try {
      ...
      const res = await axios.get(`https://instagramclone-mern-project.onrender.com/api/v1/post/${post?._id}/${action}`,{withCredentials:true})
      if(res.data.success){
        setLiked(!liked);
  ...
  try {
    const res = await axios.post(
      `https://instagramclone-mern-project.onrender.com/api/v1/post/${post?._id}/comment`,
      { text },
      {
        headers: {
  ...
  const bookmarkHandler = async () =>{
    try {
      const res = await axios.get(`https://instagramclone-mern-project.onrender.com/api/v1/post/${post?._id}/bookmark`,{withCredentials:true});
      if(res.data.success){
        toast.success(res.data.message);
      }
    } catch (error) {
